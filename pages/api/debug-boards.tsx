...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  try {
    ...
    if (req.query.addToBoard && req.query.boardId) {
      ...
      if (isUserInBoard) {
        ...
        const result = await db
          .collection('boards')
          .updateOne({ _id: boardId }, { $push: { users: String(userId) } });
    ...
    const personalBoards = await db.collection('boards').find({ createdBy: userId }).toArray();
    ...
    return res
      .status(500)
      .json({ message: 'Internal server error', status: 500, error: String(error) });
  }
}
